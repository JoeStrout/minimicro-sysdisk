...
- ☑︎ bug (command line only): printing a map containing a function reference fails to print FUNCTION() like the C# version.
- ☐ bug: when entering a `for` loop at the REPL, C++ MiniScript immediately executes the first time through, while C# MiniScript waits until `end for` to execute even once; and with an if block, it'll execute the body even if the condition is false! (CANNOT REPRODUCE)
...
- ☑︎ bug (command line): the result of `stackTrace` is reversed; the most immediate call frame should be first in the list, rather than last.
