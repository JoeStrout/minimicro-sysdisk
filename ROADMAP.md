...
- ☑︎ bug: when entering a `for` loop at the REPL, C++ MiniScript immediately executes the first time through, while C# MiniScript waits until `end for` to execute even once; and with an if block, it'll execute the body even if the condition is false!
